...
class CalibrationBase:
    def __init__(self, cols=4, rows=11, patternSize_m=0.03, patternType='asymmetric_circles'):
    ...
    def detect(self, frame, refine=False):
        ...
        if self.patternType == 'chessboard':
            ...
            patternFound, corners = cv2.findChessboardCorners(frame, (self.cols,self.rows))
            if patternFound and refine:
                # Implement cornerSubPix here
                patternFound, corners = cv2.findChessboardCornersSB(frame, (self.cols,self.rows))

        elif self.patternType == 'symmetric_circles':
            # Implement findCirclesGrid here
            patternFound, corners = cv2.findCirclesGrid(frame, (self.cols,self.rows), None,flags=cv2.CALIB_CB_SYMMETRIC_GRID)
        elif self.patternType == 'asymmetric_circles':
            # Implement findCirclesGrid here
            patternFound, corners = cv2.findCirclesGrid(frame, (self.cols,self.rows),None,flags=cv2.CALIB_CB_ASYMMETRIC_GRID)
...
class MonoCalibration(CalibrationBase):
    def acquire(self, cameraId=2, startIndex=0):
        ...
        while (True):
            ...
            self.patternType = list(self.patternTypes.keys())[
                list(self.patternTypes.values()).index(cv.getTrackbarPos(patternTypeName, 'Camera'))]
    ...
    def calibrate(self, framesPath='results/acquired'):
        ...
        self.rms,self.cameraMatrix,self.distCoeffs,self.rvecs,self.tvecs,stdIntrin,stdExtrin,self.perViewErrors = cv2.calibrateCameraExtended(objectPoints,imgPoints,imageSize,None,None)
...
class StereoCalibration(CalibrationBase):
    def calibrate(self, framesPathLeft='data/stereo/calibration/left', framesPathRight='data/stereo/calibration/right'):
        workingImagesLeft, objectPointsLeft, imgPointsLeft, imageSizeLeft = self.detectInImages(framesPathLeft,
                                                                                                'results/calibration/left')
        workingImagesRight, objectPointsRight, imgPointsRight, imageSizeRight = self.detectInImages(framesPathRight,
                                                                                                    'results/calibration/right')
        ...
        self.rms, self.cameraMatrixLeft, self.distCoeffsLeft, self.cameraMatrixRight, self.distCoeffsRight, self.R, self.T, self.E, self.F, self.perViewErrors = cv2.stereoCalibrateExtended(objectPointsLeft,imgPointsLeft,imgPointsRight,None,None,None,None,imageSizeLeft,None,None,None,None,None,cv2.CALIB_ZERO_DISPARITY+cv2.CALIB_FIX_K3,self.criteria)
