...
class MonoRectification:
    ...
    def computeCorrectionMaps(self, alpha=1.0, xRatio=1, yRatio=1):
        ...
        retval, validPixRoi = cv2.getOptimalNewCameraMatrix(self.cameraMatrix, self.distCoeffs, newImageSize, alpha)
        self.map1, self.map2 = cv2.initUndistortRectifyMap(self.cameraMatrix, self.distCoeffs, None, retval, newImageSize,cv2.CV_32FC1)
    ...
    def display(self, cameraId=2, fps=25):
...
class StereoRectification:
    ...
    def computeCorrectionMaps(self, alpha=1.0, ratio=1):
        ...
        RLeft, RRight, PLeft, PRight, Q, validPixROI1, validPixROI2 = cv2.stereoRectify(self.cameraMatrixLeft,self.distCoeffsLeft,self.cameraMatrixRight,self.distCoeffsRight,newImageSize,self.R,self.T,flags=cv2.CALIB_ZERO_DISPARITY+cv2.CALIB_FIX_K3)

        self.mapxLeft, self.mapyLeft = cv.initUndistortRectifyMap(self.cameraMatrixLeft, self.distCoeffsLeft, RLeft,
                                                                  PLeft, newImageSize, cv.CV_32FC1)
        self.mapxRight, self.mapyRight = cv.initUndistortRectifyMap(self.cameraMatrixRight, self.distCoeffsRight,
                                                                    RRight, PRight, newImageSize, cv.CV_32FC1)
    ...
    def display(self, left, right):
        ...
        while (True):
            ...
            for i in range(0, self.epipolarLines):
                display = cv.line(display, (0, int(i * display.shape[1::-1][1] / self.epipolarLines)),
                                  (display.shape[1::-1][0] - 1, int(i * display.shape[1::-1][1] / self.epipolarLines)),
                                  (0, 0, 200))
